#ifndef DISNEY_DIELECTRIC_H
#define DISNEY_DIELECTRIC_H
...
vec3 SampleDisneyDielectric(Material m, vec3 wo, out vec3 wi, out float pdf_w, out float cos_theta, vec2 xi)
{
	wi = vec3(0);
	pdf_w = 0.0;
	cos_theta = 0.0; 
    ...
    int thin = GetDisneyThin(m);
    float roughness = GetDisneyRoughness(m);
    float ior = GetDisneyIOR(m);

	float a = (thin == 1) ? modify_thin_roughness(ior, roughness) : roughness;
	float alpha = a * a;

	bool has_reflection = bsdf_has_property(bsdf_props, BSDF_FLAG_REFLECTION);
	bool has_transmission = bsdf_has_property(bsdf_props, BSDF_FLAG_TRANSMISSION); 

	if (( ior == 1.0 && thin == 0) || bsdf_is_effectively_delta(alpha)) {	// Perfect reflection/transmission
		float F = fresnel_dielectric(wo.z, ior );  
		...
		if (is_reflection) {
			...
			if (!Refract(vec3(0, 0, 1), wo, ior, 1, wi, f)) {
				return vec3(0);
			}
			cos_theta = wi.z;
	...
	float F = fresnel_dielectric(dot(wo, h), ior);
	...
	if (is_reflection) { //反射部分
		...
		if ( thin == 1 ) {
			wi = reflect(-wo, h);
			base_col = sqrt(albedo);
			possibly_modified_inv_eta = ior;
		} else {
			tir = !Refract(h, wo, ior, 1, wi, f, possibly_modified_inv_eta);
			base_col = albedo;
		}
		...
		f = base_col * f * (1.0 - F) * D * G_GGX_correlated_isotropic(alpha, wo, wi) * abs(dot(wi, h) * dot(wo, h) / (wi.z * wo.z * jacobian_denom));
	}
	cos_theta = wi.z;
	return f;
}

vec3 EvalDisneyDielectric(Material m, vec3 wo, vec3 wi, out float pdf_w, out float pdf_rev_w, bool forward_facing, uint mode, bool eval_reverse_pdf)
{
	pdf_w = 0.0;
	pdf_rev_w = 0.0;
	...
	if (!is_reflection) {
		eta = forward_facing ? ior : 1.0 / ior;
	}
	...
	if (is_reflection) {
		...
		jacobian_denom = jacobian_denom * jacobian_denom; 
		float jacobian = abs(dot(wi, h)) / jacobian_denom; 
		vec3 base_col = thin == 1 ? sqrt(albedo) : albedo;
...
float EvalDisneyDielectric_PDF(Material m, vec3 wo, vec3 wi, bool forward_facing)
{
    int bsdf_props = GetDisneyBSDFProperties(m);
    int thin = GetDisneyThin(m);
    float roughness = GetDisneyRoughness(m);
