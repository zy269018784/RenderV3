...
VulkanRHI::VulkanRHI(int InGpuIndex) //无窗口模式
	: GpuIndex(InGpuIndex)
{ 
...
#ifdef USE_QT
VulkanRHI::VulkanRHI(QVulkanWindow* window)
{	
...
VulkanRHI::~VulkanRHI()
{ 	
	//std::cout << "VulkanRHI::~VulkanRHI()" << std::endl;
...
VkShaderModule VulkanRHI::ReadShader(std::string filename)
{
	size_t shaderSize;
...
/*
	1. Resource Creation
*/
RHIVertexShader* VulkanRHI::RHILoadVertexShader(const char* filename)
{
...
RHIPixelShader* VulkanRHI::RHILoadPixelShader(const char* filename)
{
	VkShaderModule module = ReadShader(filename);
	if (module == VK_NULL_HANDLE)
		return nullptr;
...
RHIComputeShader* VulkanRHI::RHILoadComputeShader(const char* filename)
{
	VkShaderModule module = ReadShader(filename);
	if (module == VK_NULL_HANDLE)
		return nullptr;
...
RHIRayGenShader* VulkanRHI::RHILoadRayGenShader(const char* filename)
{
	VkShaderModule module = ReadShader(filename);
	if (module == VK_NULL_HANDLE)
		return nullptr;
...
RHIRayMissShader* VulkanRHI::RHILoadRayMissShader(const char* filename)
{
	VkShaderModule module = ReadShader(filename);
	if (module == VK_NULL_HANDLE)
		return nullptr;
...
RHIRayHitGroupShader* VulkanRHI::RHILoadRayHitGroupShader(const char* filename, const char* filenameAnyhit, const char* filenameIntersect)
{
...
RHIVertexShader* VulkanRHI::RHICreateVertexShader(std::vector<char> GlslCode)
{
...
// create pixel shader
RHIPixelShader* VulkanRHI::RHICreatePixelShader(std::vector<char> GlslCode)
{
...
RHIComputeShader* VulkanRHI::RHICreateComputeShader(std::vector<char> GlslCode)
{
...
// create 1D texture 
RHITexture* VulkanRHI::RHICreateTexture1D(std::uint32_t SizeX, std::uint8_t Format, std::uint32_t NumMips, TextureCreateFlags Flags, RHIAccess InResourceState)
{
...
// create 2D texture 
RHITexture* VulkanRHI::RHICreateTexture2D(std::uint32_t SizeX, std::uint32_t SizeY, std::uint8_t Format, std::uint32_t NumMips, TextureCreateFlags Flags, RHIAccess InResourceState)
{
...
// create 3D texture 
RHITexture* VulkanRHI::RHICreateTexture3D(std::uint32_t SizeX, std::uint32_t SizeY, std::uint32_t SizeZ, std::uint8_t Format, std::uint32_t NumMips, TextureCreateFlags Flags, RHIAccess InResourceState)
{
...
RHITexture* VulkanRHI::RHICreateTextureCube(std::uint32_t Size, std::uint8_t Format, std::uint32_t NumMips, TextureCreateFlags Flags, RHIAccess InResourceState)
{
...
RHITexture* VulkanRHI::RHICreateTexture2DArray(std::uint32_t SizeX, std::uint32_t SizeY, std::uint32_t SizeZ, std::uint8_t Format, std::uint32_t NumMips, std::uint32_t NumSamples, TextureCreateFlags Flags, RHIAccess InResourceState)
{
...
RHITexture* VulkanRHI::RHICreateTextureCubeArray(std::uint32_t SizeX, std::uint32_t SizeY, std::uint32_t SizeZ, std::uint8_t Format, std::uint32_t NumMips, std::uint32_t NumSamples, TextureCreateFlags Flags, RHIAccess InResourceState)
{
...
// create buffer
RHIBuffer* VulkanRHI::RHICreateBuffer(std::uint32_t Size, BufferUsageFlags Usage, std::uint32_t Stride, const void* Data)
{
...
RHISampler* VulkanRHI::RHINewSampler()
{
...
RHIRaytracingBLAS* VulkanRHI::RHINewRaytracingBLAS(RHIRaytracingGeometryData* geoData)
{
	VulkanRaytracingBLAS* blas = new VulkanRaytracingBLAS(Device, geoData);
	return static_cast<RHIRaytracingBLAS*>(blas);
}

RHIRaytracingBLAS* VulkanRHI::RHINewRaytracingBLAS(RHIRaytracingGeometryDataSimple* geoData)
{
	VulkanRaytracingBLAS* blas = new VulkanRaytracingBLAS(Device, geoData);
	return static_cast<RHIRaytracingBLAS*>(blas);
}

std::vector< RHIRaytracingBLAS* > VulkanRHI::RHICreateRaytracingBLASs(std::vector< RHIRaytracingGeometryData* >& geometries)
{
...
VulkanCommandBuffer* VulkanRHI::BeginCommand()
{
...
void VulkanRHI::EndCommand(VulkanCommandBuffer* CmdBuffer)
{
	CmdBuffer->End();
...
void VulkanRHI::InitMultiThreadCommandBuffers(int threads)
{
...
void VulkanRHI::DestroyMultiThreadCommandBuffers()
{
	for (int i = 0; i < cmdBuffers.size(); i++) {
		delete cmdBuffers[i];
	}
...
void VulkanRHI::RHIBuildBLASs(std::vector< RHIRaytracingBLAS* >& BLASs)
{
#if 1
...
RHIRaytracingTLAS* VulkanRHI::RHINewRaytracingTLAS(uint64_t deviceAddress)
{
	VulkanRaytracingTLAS* tlas = new VulkanRaytracingTLAS(Device, deviceAddress);
	return static_cast<RHIRaytracingTLAS*>(tlas);
}

RHIRaytracingTLAS* VulkanRHI::RHINewRaytracingTLAS(std::vector< RHIRaytracingBLAS* >& BLASs)
{
	VulkanRaytracingTLAS* tlas = new VulkanRaytracingTLAS(Device, BLASs);
	return static_cast<RHIRaytracingTLAS*>(tlas);
}

RHIRaytracingTLAS* VulkanRHI::RHINewRaytracingTLAS(std::vector< RHIRaytracingBLAS* >& BLASs, std::vector<glm::mat4> transforms)
{
	VulkanRaytracingTLAS* tlas = new VulkanRaytracingTLAS(Device, BLASs, transforms);
	return static_cast<RHIRaytracingTLAS*>(tlas);
}
 
RHIShaderBindingTables* VulkanRHI::RHINewShaderBindingTables(RHIRaytracingPipeline* pipeline)
{
...
*/
// update buffer
void VulkanRHI::RHIUpdateBuffer(RHIBuffer* Buffer, std::uint32_t Offset, std::uint32_t Size, const void* InData)
{
...
// copy buffer
void VulkanRHI::RHICopyBuffer(RHIBuffer* SourceBufferRHI, RHIBuffer* DestBufferRHI)
{
...
void VulkanRHI::RHIDispatchComputeShader(std::uint32_t ThreadGroupCountX, std::uint32_t ThreadGroupCountY, std::uint32_t ThreadGroupCountZ)
{
...
*/
// update 1D texture
void VulkanRHI::RHIUpdateTexture1D(RHITexture1D* TextureRHI, uint32_t MipIndex, const RHIUpdateTextureRegion1D& UpdateRegionIn, const uint8_t* SourceDataIn)
{
...
// update 2D texture
void VulkanRHI::RHIUpdateTexture2D(RHITexture2D* TextureRHI, std::uint32_t MipIndex, const RHIUpdateTextureRegion2D& UpdateRegionIn, std::uint32_t SourcePitch, const std::uint8_t* SourceDataIn)
{
	const void* InData = SourceDataIn;
...
// update 3D texture
void  VulkanRHI::RHIUpdateTexture3D(RHITexture3D* TextureRHI, std::uint32_t MipIndex, const RHIUpdateTextureRegion3D& UpdateRegion, std::uint32_t SourceRowPitch, std::uint32_t SourceDepthPitch, const std::uint8_t* SourceDataIn)
{
	const void* InData = SourceDataIn;
...
void VulkanRHI::RHIUpdateTextureCube(RHITextureCube* TextureRHI, CubeMapFace Face, std::uint32_t MipIndex, const RHIUpdateTextureRegion2D& UpdateRegionIn, std::uint32_t SourcePitch, const std::uint8_t* SourceDataIn)
{
	const void* InData = SourceDataIn;
...
void  VulkanRHI::RHIUpdateTexture2DArray(RHITexture2DArray* TextureRHI, std::uint32_t MipIndex, std::uint32_t ArrayIndex, const RHIUpdateTextureRegion2D& UpdateRegionIn, std::uint32_t SourcePitch, const std::uint8_t* SourceDataIn)
{
	const void* InData = SourceDataIn;
...
void  VulkanRHI::RHIUpdateTextureCubeArray(RHITextureCubeArray* TextureRHI, CubeMapFace Face, std::uint32_t MipIndex, std::uint32_t ArrayIndex, const RHIUpdateTextureRegion2D& UpdateRegionIn, std::uint32_t SourcePitch, const std::uint8_t* SourceDataIn)
{
	const void* InData = SourceDataIn;
...
bool VulkanRHI::RecreateSwapChain(void)
{
...
*/
RHICommandBuffer* VulkanRHI::RHIBeginFrame()
{   
...
RHICommandBuffer* VulkanRHI::RHIBeginOffscreenFrame()
{
	std::vector<VkFence> fences;
	fences.resize(InFlightFences.size());
	for (int i = 0; i < InFlightFences.size(); ++i) {
