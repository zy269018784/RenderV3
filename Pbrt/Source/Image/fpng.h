...
namespace fpng
{
	...
	const std::uint32_t FPNG_CRC32_INIT = 0;
	std::uint32_t fpng_crc32(const void* pData, size_t size, std::uint32_t prev_crc32 = FPNG_CRC32_INIT);

	// Fast Adler32 SSE4.1 Adler-32 with a scalar fallback.
	const std::uint32_t FPNG_ADLER32_INIT = 1;
	std::uint32_t fpng_adler32(const void* pData, size_t size, std::uint32_t adler = FPNG_ADLER32_INIT);
	...
	bool fpng_encode_image_to_memory(const void* pImage, std::uint32_t w, std::uint32_t h, std::uint32_t num_chans, std::vector<uint8_t>& out_buf, std::uint32_t flags = 0);

#ifndef FPNG_NO_STDIO
	// Fast PNG encoding to the specified file.
	bool fpng_encode_image_to_file(const char* pFilename, const void* pImage, std::uint32_t w, std::uint32_t h, std::uint32_t num_chans, std::uint32_t flags = 0);
#endif
	...
	int fpng_get_info(const void* pImage, std::uint32_t image_size, std::uint32_t& width, std::uint32_t& height, std::uint32_t& channels_in_file);
	...
	// If FPNG_DECODE_NOT_FPNG is returned, you must decompress the file with a general purpose PNG decoder.
	// If another error occurs, the file is likely corrupted or invalid, but you can still try to decompress the file with another decoder (which will likely fail).
	int fpng_decode_memory(const void* pImage, std::uint32_t image_size, std::vector<uint8_t>& out, std::uint32_t& width, std::uint32_t& height, std::uint32_t& channels_in_file, std::uint32_t desired_channels);

#ifndef FPNG_NO_STDIO
	int fpng_decode_file(const char* pFilename, std::vector<uint8_t>& out, std::uint32_t& width, std::uint32_t& height, std::uint32_t& channels_in_file, std::uint32_t desired_channels);
#endif

	// ---- Internal API used for Huffman table training purposes

#if FPNG_TRAIN_HUFFMAN_TABLES
	const std::uint32_t HUFF_COUNTS_SIZE = 288;
	extern uint64_t g_huff_counts[HUFF_COUNTS_SIZE];
	bool create_dynamic_block_prefix(uint64_t* pFreq, std::uint32_t num_chans, std::vector<uint8_t>& prefix, uint64_t& bit_buf, int& bit_buf_size, std::uint32_t *pCodes, uint8_t *pCodesizes);
#endif
