...
namespace Render {
    ...
    CPU_GPU inline uint64_t MurmurHash64A(const unsigned char* key, size_t len,
        uint64_t seed) {
    ...
    CPU_GPU inline uint64_t MixBits(uint64_t v);
    ...
    template <typename T>
    CPU_GPU inline uint64_t HashBuffer(const T* ptr, size_t size, uint64_t seed = 0) {
        return MurmurHash64A((const unsigned char*)ptr, size, seed);
    }

    template <typename... Args>
    CPU_GPU inline uint64_t Hash(Args... args);

    template <typename... Args>
    CPU_GPU inline void hashRecursiveCopy(char* buf, Args...);

    template <>
    CPU_GPU inline void hashRecursiveCopy(char* buf) {}

    template <typename T, typename... Args>
    CPU_GPU inline void hashRecursiveCopy(char* buf, T v, Args... args) {
    ...
    template <typename... Args>
    CPU_GPU inline Float HashFloat(Args... args) {
