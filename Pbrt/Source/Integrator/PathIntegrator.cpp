...
namespace Render {
    PathIntegrator::PathIntegrator(Primitive aggregate, Scene* scene) {
        this->aggregate = aggregate;
        this->scene = scene;
    }

    bool PathIntegrator::IntersectP(const Ray& ray, Float tMax) const {
    ...
    bool PathIntegrator::Unoccluded(const Interaction& p0, const Interaction& p1)  const {
        Ray ray;
    ...
    bool PathIntegrator::IntersectTransparent(const Ray& ray, Float tMax, RGB& beta) const {
        auto intr = aggregate.Intersect(ray, tMax);
    ...
    bool PathIntegrator::ThroughTransparent(const Interaction& p0, const Interaction& p1, RGB& beta)  const {
        Ray ray;
        ray.o = p0.p();
        ray.d = Normalize(p1.p() - p0.p());
        Float tMax = Length(p1.p() - p0.p());
    ...
    CPU_GPU RGB PathIntegrator::SampleLd(const LightSampleContext& ctx, const SurfaceInteraction& intr, const Ray& ray) const {
    ...
    Optional<ShapeIntersection> PathIntegrator::Intersect(const Ray& ray,
        Float tMax) const {
        if (aggregate)
    ...
    RGB PathIntegrator::Li(const Ray& ray, Float u, Float tMax) const {
        RGB beta(1, 1, 1);
        int depth = 5;
        Ray currentRay = ray;
            ...
            if (intr) {
                ...
                if (tag > 0) {
                    ...
                    // 花岗岩
                        ...
                        {
                            beta *= sample.f;
                            //auto Color = intr->intr.material.Evaluate(intr->intr, lambda);
                            //beta *= Color;
                            currentRay.d = Normalize(sample.wi);
                            currentRay.o = intr->intr.p();
                        }
    ...
    RGB PathIntegrator::LiPixel(int row, int col) const {
    ...
    RGB PathIntegrator::LiGlass(const Ray& ray) const {
        return Li(ray, 0.f) + Li(ray, 1.f);
    }

    RGB PathIntegrator::LiGranite(const Ray& ray) const {
        return Li(ray, 0.f) + Li(ray, 1.f);
    }

    // tag > 0: 有材质
    unsigned int PathIntegrator::FirstMaterialTag(const Ray& ray, Float tMax) const {
        unsigned int tag = 0;
