...
namespace Render {
    TestIntegrator2::TestIntegrator2(Primitive aggregate, Scene* scene) {
        this->aggregate = aggregate;
        this->scene = scene;
    }

    bool TestIntegrator2::IntersectP(const Ray& ray, Float tMax) const {
    ...
    bool TestIntegrator2::Unoccluded(const Interaction& p0, const Interaction& p1)  const {
        Ray ray;
    ...
    bool TestIntegrator2::IntersectTransparent(const Ray& ray, Float tMax, RGB& beta) const {
        auto intr = aggregate.Intersect(ray, tMax);
    ...
    bool TestIntegrator2::ThroughTransparent(const Interaction& p0, const Interaction& p1, RGB& beta)  const {
        Ray ray;
        ray.o = p0.p();
        ray.d = Normalize(p1.p() - p0.p());
        Float tMax = Length(p1.p() - p0.p());
    ...
    CPU_GPU RGB TestIntegrator2::SampleLd(const LightSampleContext& ctx, const SurfaceInteraction& intr, const Ray& ray) const {
    ...
    Optional<ShapeIntersection> TestIntegrator2::Intersect(const Ray& ray,
        Float tMax) const {
        if (aggregate)
    ...
    RGB TestIntegrator2::Li(const Ray& ray, Float u, Float tMax) const {
        RGB beta(1, 1, 1);
        int depth = 5;
        Ray currentRay = ray;
    ...
    RGB TestIntegrator2::LiPixel(int row, int col) const {
    ...
    bool TestIntegrator2::IntersetGlass(const Ray& ray, Float tMax) const {
