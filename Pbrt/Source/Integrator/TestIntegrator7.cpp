...
namespace Render {
    TestIntegrator7::TestIntegrator7(Primitive aggregate, Scene* scene) {
        this->aggregate = aggregate;
        this->scene = scene;
    }

    bool TestIntegrator7::IntersectP(const Ray& ray, Float tMax) const {
    ...
    bool TestIntegrator7::Unoccluded(const Interaction& p0, const Interaction& p1)  const {
        Ray ray;
    ...
    bool TestIntegrator7::IntersectTransparent(const Ray& ray, Float tMax, RGB& beta) const {
        auto intr = aggregate.Intersect(ray, tMax);
    ...
    bool TestIntegrator7::ThroughTransparent(const Interaction& p0, const Interaction& p1, RGB& beta)  const {
        Ray ray;
        ray.o = p0.p();
        ray.d = Normalize(p1.p() - p0.p());
        Float tMax = Length(p1.p() - p0.p());
    ...
    CPU_GPU RGB TestIntegrator7::SampleLd(const LightSampleContext& ctx, const SurfaceInteraction& intr, const Ray& ray) const {
    ...
    Optional<ShapeIntersection> TestIntegrator7::Intersect(const Ray& ray,
        Float tMax) const {
        if (aggregate)
    ...
    RGB TestIntegrator7::Li(const Ray& ray, Float u, Float tMax) const {
        RGB beta(1, 1, 1);
        int depth = 5;
        Ray currentRay = ray;
    ...
    RGB TestIntegrator7::LiPixel(int row, int col) const {
        ...
        for (int i = 0; i < samples; i++) {
            ...
            acc_color[2] += final.b;
#else
            //final.r = final.r / (final.r + 1e5);
            //final.g = final.g / (final.g + 1e5);
            //final.b = final.b / (final.b + 1e5);
        ...
        // acc_color[0] = 255 * acc_color[0] / (acc_color[0] + 1e5);
        // acc_color[1] = 255 * acc_color[1] / (acc_color[1] + 1e5);
        // acc_color[2] = 255 * acc_color[2] / (acc_color[2] + 1e5);

        acc_color[0] = 255 * acc_color[0] / samples / 1e5;
        acc_color[1] = 255 * acc_color[1] / samples / 1e5;
        acc_color[2] = 255 * acc_color[2] / samples / 1e5;
#else
        acc_color[0] = acc_color[0] * 255 / samples;
        acc_color[1] = acc_color[1] * 255 / samples;
        acc_color[2] = acc_color[2] * 255 / samples;
#endif

        //acc_color[0] = acc_color[0]  / samples;
        //acc_color[1] = acc_color[1]  / samples;
        //acc_color[2] = acc_color[2]  / samples;
