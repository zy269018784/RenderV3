...
namespace Render {
	using std::sqrt;
	CPU_GPU Float inline IntersectSphere(Vector3f origin, Vector3f dir, Vector3f spherePos, Float sphereRad)
	{
	...
	/*
		...
		所以N * (o + t * d) - N * a = 0
			...
			t = (N * a - N * o) / (N * d)
	*/
	CPU_GPU inline Float IntersectTriangle(Vector3f o, Vector3f d, Vector3f a, Vector3f b, Vector3f c)
	{
		Vector3f ab = b - a;
		Vector3f ac = c - a;
		Vector3f bc = c - b;
