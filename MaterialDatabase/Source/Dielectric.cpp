...
static int DielectricCallback(void* NotUsed, int argc, char** argv, char** azColName)
{
	std::string pbrt;
...
{
	char* sql = "select * from DielectricParameter;";

	std::fstream file;
	file.open("material/Dielectric.pbrt", std::ios::out);
	sqlite3_exec(db, sql, DielectricCallback, (void*)&file, nullptr);
	file.close();
}